...
// massage-active-auth V2
const FULL_TIME = 10 * 60;
let currentTime = 9 * 60 + 57;
...
circle.style.strokeDashoffset = circumference * (1 - currentTime / FULL_TIME);
...
function updateTimer() {
  ...
  mainTimer.textContent = timeText;
  bottomTimer.textContent = timeText;

  const progress = currentTime / FULL_TIME;
...
function checkBottomTimerState() {
  const timerRect = topTimerBlock.getBoundingClientRect();

  if (timerRect.bottom <= 0) {
    bottomPanel.classList.add('_timer-visible');
  } else {
    bottomPanel.classList.remove('_timer-visible');
  }
}
...
window.addEventListener('scroll', checkBottomTimerState);
window.addEventListener('load', checkBottomTimerState);